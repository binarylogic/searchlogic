      ...
      private 
        def create_scope(curr_scope, condition, value)
          ...
          if (scope_lambda.try(:[], :type) == :boolean && value == true) || ordering?(condition)
            curr_scope.__send__(std_condition)
          elsif scope_lambda.try(:[], :scope).try(:arity) == 1
            curr_scope.__send__(std_condition, value)
          else
            [value].flatten.size == 1 ? curr_scope.__send__(std_condition, value) : curr_scope.__send__(std_condition, *value) 
          end
        end
