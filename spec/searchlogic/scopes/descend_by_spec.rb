...
describe Searchlogic::ActiveRecordExt::Scopes::Conditions::DescendBy do 
  ...
  it "orders on correct column when scope contains joins values before and also within the ordering" do 
    ...
    search = Company.search(:users_orders_line_items_price_lte => "2",  :order => :descend_by_orders_line_items_id )
    search.all.should eq([c3,c1])
    search = LineItem.search(:order_user_name_equal => "James", :order => :descend_by_order_total)
    search.all.should eq([l1])
  end
end